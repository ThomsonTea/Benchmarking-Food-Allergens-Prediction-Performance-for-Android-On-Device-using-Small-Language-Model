...
class ResultsAdapter : RecyclerView.Adapter<ResultsAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        // === TABLE 3: SAFETY-ORIENTED METRICS - ✅ FIXED ===

        // Hallucination
        holder.hallucinationText.text = if (result.hallucinationCount > 0) {
            "Hallucination: Yes (${result.hallucinationCount} - ${result.hallucinatedAllergens})"
        } else {
            "Hallucination: No"
        }

        // Over-Prediction
        holder.overPredictionText.text = if (result.overPredictionCount > 0) {
            "Over-Prediction: Yes (${result.overPredictionCount} - ${result.overPredictedAllergens})"
        } else {
            "Over-Prediction: No"
        }
